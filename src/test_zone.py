...
# Build the best calibrated model: XGBoost + Platt scaling (sigmoid). We wrap an XGBClassifier estimator inside CalibratedClassifierCV.
...
def run_comparison(random_state: int = 42) -> None:
    ...
    # 5. Train calibrated XGBoost model (Platt scaling)
    print("  -> Training Calibrated (Platt) XGBoost model ...")
    clf = build_calibrated_xgboost(random_state=random_state)
